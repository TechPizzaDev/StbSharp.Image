...
namespace StbSharp.ImageRead
{
    ...
    public static class Bmp
    {
        ...
        public static BmpInfo Info(ImageBinReader reader, out ReadState state)
        {
            ...
            if (!ParseHeader(reader, state, out BmpInfo info))
                StbImageReadException.Throw(ErrorCode.UnknownHeader);

            return info;
        }

        public static bool ParseHeader(ImageBinReader s, ReadState ri, out BmpInfo info)
        {
            if (s == null)
            ...
            if (s.ReadInt16LE() != 1)
                StbImageReadException.Throw(ErrorCode.BadColorPlane);

            info.bitsPerPixel = s.ReadInt16LE();
            if (info.bitsPerPixel == 1)
                StbImageReadException.Throw(ErrorCode.MonochromeNotSupported);

            if (info.headerSize != 12)
            {
                int compress = s.ReadInt32LE();
                if ((compress == 1) || (compress == 2))
                    StbImageReadException.Throw(ErrorCode.RLENotSupported);
                    ...
                    if ((info.bitsPerPixel == 16) || (info.bitsPerPixel == 32))
                    {
                        ...
                        else if (compress == 3)
                        {
                            ...
                            if ((info.mr == info.mg) && (info.mg == info.mb))
                                StbImageReadException.Throw(ErrorCode.BadMasks);
                        }
                        else
                        {
            ...
            ImageBinReader s, ReadState ri, ArrayPool<byte>? bytePool = null)
        {
            ...
            if (!ParseHeader(s, ri, out BmpInfo info))
                StbImageReadException.Throw(ErrorCode.UnknownHeader);
