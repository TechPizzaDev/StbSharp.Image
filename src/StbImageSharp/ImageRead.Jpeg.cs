    ...
    public static partial class ImageRead
    {
        public static unsafe class Jpeg
        {
            ...
            public static int BuildHuffman(ref Huffman h, int* count)
            {
                ...
                for (j = 1; j <= 16; ++j)
                {
                    ...
                    if (h.size[k] == j)
                    {
                        ...
                        if ((code - 1) >= (1 << j))
                            return (int)Error("bad code lengths");
                    }
            ...
            public static void GrowBufferUnsafe(Context j)
            {
                do
                {
                    int b = (int)(j.nomore != 0 ? 0 : j.s.ReadByte());
                    if (b == 0xff)
                    {
                        int c = (int)j.s.ReadByte();
                        while (c == 0xff)
                            c = (int)j.s.ReadByte();
                ...
                short[] fac, int b, ushort[] dequant)
            {
                ...
                if (j.code_bits < 16)
                    GrowBufferUnsafe(j);
                t = (int)HuffmanDecode(j, ref hdc);
                if (t < 0)
                    return (int)Error("bad huffman code");
                CRuntime.MemSet(data, 0, 64 * sizeof(short));
                diff = (int)(t != 0 ? ExtendReceive(j, (int)t) : 0);
                dc = (int)(j.components[b].dc_pred + diff);
                j.components[b].dc_pred = (int)dc;
                ...
                do
                {
                    ...
                    {
                        int rs = (int)HuffmanDecode(j, ref hac);
                        if (rs < 0)
                            return (int)Error("bad huffman code");
                        s = (int)(rs & 15);
                        r = (int)(rs >> 4);
                        ...
                        else
                        {
                            k += (int)r;
                            zig = (uint)DeZigZag[k++];
                            data[zig] = (short)(ExtendReceive(j, (int)s) * dequant[zig]);
                        }
                    }
                } while (k < 64);
                ...
                Context j, short* data, ref Huffman hdc, int b)
            {
                ...
                int t;
                if (j.spec_end != 0)
                    return (int)Error("can't merge dc and ac");
                if (j.code_bits < 16)
                    GrowBufferUnsafe(j);
                if (j.succ_high == 0)
                {
                        ...
                        else
                        {
                            ...
                            if (s == 0)
                            {
                                if (r < 15)
                                {
                                    j.eob_run = (int)(1 << r);
                                    if (r != 0)
                                        j.eob_run += (int)ReadBits(j, (int)r);
                                    --j.eob_run;
                                    break;
                                }
                            ...
                            else
                            {
                                k += (int)r;
                                zig = (uint)DeZigZag[k++];
                                data[zig] = (short)(ExtendReceive(j, (int)s) << shift);
                            }
                        }
                    } while (k <= j.spec_end);
                }
            ...
            public static void IdctBlock(byte* _out_, int out_stride, short* data)
            {
                ...
                for (i = 0, v = val, o = _out_; i < 8; ++i, v += 8, o += out_stride)
                {
                    ...
                    o[0] = (byte)Clamp((int)((x0 + t3) >> 17));
                    o[1] = (byte)Clamp((int)((x1 + t2) >> 17));
                    o[2] = (byte)Clamp((int)((x2 + t1) >> 17));
                    o[3] = (byte)Clamp((int)((x3 + t0) >> 17));
                    o[4] = (byte)Clamp((int)((x3 - t0) >> 17));
                    o[5] = (byte)Clamp((int)((x2 - t1) >> 17));
                    o[6] = (byte)Clamp((int)((x1 - t2) >> 17));
                    o[7] = (byte)Clamp((int)((x0 - t3) >> 17));
                }
            }
            ...
            public static bool ParseEntropyCodedData(Context z)
            {
                ...
                if (!z.progressive)
                {
                    if (z.scan_n == 1)
                    {
                        ...
                        int n = (int)z.order[0];
                        int w = (int)((z.components[n].x + 7) >> 3);
                        int h = (int)((z.components[n].y + 7) >> 3);
                        for (int j = 0; j < h; ++j)
                        {
                            for (int i = 0; i < w; ++i)
                            {
                                int ha = (int)z.components[n].ha;
                                if (DecodeBlock(
                                        z, data, ref z.huff_dc[z.components[n].hd], ref z.huff_ac[ha],
                                        z.fast_ac[ha], (int)n, z.dequant[z.components[n].tq]) == 0)
                                    return false;

                                z.idct_block_kernel(
                                    z.components[n].data + z.components[n].w2 * j * 8 + i * 8,
                                    (int)z.components[n].w2, data);
                            ...
                            for (int i = 0; i < z.img_mcu_x; ++i)
                            {
                                for (k = 0; k < z.scan_n; ++k)
                                {
                                    ...
                                    for (y = 0; y < z.components[n].v; ++y)
                                    {
                                        ...
                                        {
                                            int x2 = (int)((i * z.components[n].h + x) * 8);
                                            int y2 = (int)((j * z.components[n].v + y) * 8);
                                            int ha = (int)z.components[n].ha;
                                            if (
                                                DecodeBlock(z, data,
                            ...
                            for (int i = 0; i < w; ++i)
                            {
                                ...
                                if (z.spec_start == 0)
                                {
                                    if (DecodeBlockProgressiveDc(z, data,
                                            ref z.huff_dc[z.components[n].hd], (int)n) == 0)
                                        return false;
                                }
                                else
                                {
                            ...
                            for (int i = 0; i < z.img_mcu_x; ++i)
                            {
                                for (k = 0; k < z.scan_n; ++k)
                                {
                                    ...
                                    for (y = 0; y < z.components[n].v; ++y)
                                    {
                                        for (x = 0; x < z.components[n].h; ++x)
                                        {
                                            int x2 = (int)(i * z.components[n].h + x);
                                            int y2 = (int)(j * z.components[n].v + y);
                                            short* data = z.components[n].coeff + 64 * (x2 + y2 * z.components[n].coeff_w);
                                            if (DecodeBlockProgressiveDc(
                                                z, data, ref z.huff_dc[z.components[n].hd], (int)n) == 0)
                                                return false;
                                        }
                                    }
                                }
            ...
            public static bool ProcessMarker(Context z, int m)
            {
                ...
                switch (m)
                {
                    ...
                    case 0xDD:
                        ...
                        z.restart_interval = (int)s.ReadInt16BE();
                        return true;

                    case 0xDB:
                        L = (int)(s.ReadInt16BE() - 2);
                        while (L > 0)
                        {
                            int q = (int)s.ReadByte();
                            int p = (int)(q >> 4);
                            int sixteen = (p != 0) ? 1 : 0;
                            int t = (int)(q & 15);
                            int i;
                    ...
                    case 0xC4:
                        ...
                        while (L > 0)
                        {
                            ...
                            for (i = 0; i < n; ++i)
                                huff[th].values[i] = (byte)s.ReadByte();
                ...
                if (((m >= 0xE0) && (m <= 0xEF)) || (m == 0xFE))
                {
                    ...
                    else if ((m == 0xEE) && (L >= 12))
                    {
                        ...
                        if (ok)
                        {
                            ...
                            s.ReadInt16BE();
                            s.ReadInt16BE();
                            z.app14_color_transform = (int)s.ReadByte();
                            L -= (int)6;
                        }
                    }
            ...
            {
                var s = z.s;
                int Ls = (int)s.ReadInt16BE();
                z.scan_n = (int)s.ReadByte();
                ...
                for (int i = 0; i < z.scan_n; ++i)
                {
                    ...
                    z.components[which].ha = (int)(q & 15);
                ...
                {
                    int aa;
                    z.spec_start = (int)s.ReadByte();
                    z.spec_end = (int)s.ReadByte();
                    aa = (int)s.ReadByte();
            ...
            public static bool ProcessFrameHeader(Context z, ScanMode scan)
            {
                ...
                z.ri.Height = (int)s.ReadInt16BE();
                if (z.ri.Height == 0)
                {
                    Error("no header height");
                    return false;
                }
                z.ri.Width = (int)s.ReadInt16BE();
                if (z.ri.Width == 0)
                {
                    Error("0 width");
                    return false;
                }
                z.ri.Components = (int)s.ReadByte();
                ...
                for (i = 0; i < z.ri.Components; ++i)
                {
                    if (z.components[i].h > h_max)
                        h_max = (int)z.components[i].h;
                    if (z.components[i].v > v_max)
                        v_max = (int)z.components[i].v;
                }
                ...
                for (i = 0; i < z.ri.Components; ++i)
                {
                    z.components[i].x = (int)((z.ri.Width * z.components[i].h + h_max - 1) / h_max);
                    z.components[i].y = (int)((z.ri.Height * z.components[i].v + v_max - 1) / v_max);
                    z.components[i].w2 = (int)(z.img_mcu_x * z.components[i].h * 8);
                    z.components[i].h2 = (int)(z.img_mcu_y * z.components[i].v * 8);
                    z.components[i].coeff = null;
                    z.components[i].raw_coeff = null;
                    z.components[i].linebuf = null;
                    z.components[i].raw_data = MAllocMad2(
                        (int)z.components[i].w2, (int)z.components[i].h2, (int)15);

                    if (z.components[i].raw_data == null)
                    {
                        FreeComponents(z, (int)(i + 1));
                        Error("outofmem");
                        return false;
                    }
                    ...
                    if (z.progressive)
                    {
                        z.components[i].coeff_w = (int)(z.components[i].w2 / 8);
                        z.components[i].coeff_h = (int)(z.components[i].h2 / 8);
                        z.components[i].raw_coeff = MAllocMad3(
                            (int)z.components[i].w2, (int)z.components[i].h2, (int)2, (int)15);
                ...
                while (!(m == 0xd9))
                {
                    if (m == 0xda)
                    {
                        ...
                        if (j.marker == 0xff)
                        {
                            ...
                            {
                                int x = (int)s.ReadByte();
                                if (x == 255)
                                {
                                    j.marker = (byte)s.ReadByte();
                                    break;
                                }
                            }
                        }
            ...
            {
                FreeComponents(j, (int)j.ri.Components);
            }
            ...
            public static IMemoryResult LoadImage(Context z)
            {
                ...
                for (k = 0; k < z.decode_n; k++)
                {
                    ...
                    r.hs = (int)(z.img_h_max / z.components[k].h);
                    r.vs = (int)(z.img_v_max / z.components[k].v);
                    r.ystep = (int)(r.vs >> 1);
                ...
                output = (byte*)MAllocMad3(z.ri.OutComponents, (int)z.ri.Width, (int)z.ri.Height, 1);
                if (output == null)
                {
                    Cleanup(z);
                    Error("outofmem");
